...
pbs_db_save_sched(void *conn, pbs_db_obj_info_t *obj, int savetype)
{
	...
	if ((pschd->db_attr_list.attr_count > 0) || (savetype & OBJ_SAVE_NEW))
		numbins++;  /* for attributes */

	as_operations_inita(&ops, numbins);
	as_key_init_str(&key, g_namespace, "scheduler", pschd->sched_name);	
	as_integer_init(&sched_savetm, db_time_now(0));
	...
	if ((pschd->db_attr_list.attr_count > 0) || (savetype & OBJ_SAVE_NEW)) {
		if (convert_db_attr_list_to_asmap(&attrmap, &pschd->db_attr_list) < 0) 
			return -1;
	...
	if (aerospike_key_operate(as, &err, &op_policy, &key, &ops, &rec) != AEROSPIKE_OK) {
		...
		db_set_error("Execution of save sched", msg, NULL);
...
load_sched(as_record* rec, pbs_db_sched_info_t *pschd)
{
	...
	if (convert_asmap_to_db_attr_list((as_hashmap *) attrmap, &pschd->db_attr_list) < 0) 
		return -1;
...
pbs_db_load_sched(void *conn, pbs_db_obj_info_t *obj)
{
	...
	if (aerospike_key_get(as, &err, NULL, &key, &p_rec) != AEROSPIKE_OK) {
		...
		db_set_error("Execution of load sched", msg, NULL);
 ...
 */
int
pbs_db_find_sched(void *conn, void *st, pbs_db_obj_info_t *obj, pbs_db_query_options_t *opts)
{
	...
	if (aerospike_query_foreach(as, &err, NULL, &query, sched_callback, state) != AEROSPIKE_OK) {
		char msg[4096];
		sprintf(msg, "aerospike_query_foreach() returned %d - %s\n", err.code, err.message);
		db_set_error("Execution of find sched", msg, NULL);
...
pbs_db_delete_sched(void *conn, pbs_db_obj_info_t *obj)
{
	...
	as_key_init_str(&key, g_namespace, "sched", sc->sched_name);
	if (aerospike_key_remove(as, &err, NULL, &key) != AEROSPIKE_OK) {
		char msg[4096];
		sprintf(msg, "aerospike_key_remove() returned %d - %s\n", err.code, err.message);
		db_set_error("Execution of delete sched", msg, NULL);
		return -1;
	}
...
pbs_db_del_attr_sched(void *conn, void *obj_id, pbs_db_attr_list_t *attr_list)
{
	...
	as_operations_inita(&ops, numbins);
	as_key_init_str(&key, g_namespace, "sched", obj_id);
	...
	if (aerospike_key_operate(as, &err, &op_policy, &key, &ops, &rec) != AEROSPIKE_OK) {
		char msg[4096];
		sprintf(msg, "aerospike_key_operate() returned %d - %s\n", err.code, err.message);
		db_set_error("Execution of delete sched attrs", msg, NULL);
		as_operations_destroy(&ops);
		as_record_destroy(rec);
		return -1;
	}
 ...
 */
void
pbs_db_reset_sched(pbs_db_obj_info_t *obj)
{
